...
class TableHelper(private val webCommand: WebCommand) {
    ...
    private val tableHeaderLocators = listOf(
    ...
    private val formElementLocator = ".//*[" +
        ...
        " name()='img'" +
        "]"
    private val csvSafeReplacement = ListOrderedMap.listOrderedMap(
        mapOf(
            " \n " to " ",
            " \r\n " to " ",
            " \r " to " ",
    ...
    fun saveDivsAsCsv(
        ...
        while (true) {
            ...
            if (!clickNextPage(nextPageLocator) { _ ->
    ...
    fun assertTable(locator: String, row: String, column: String, text: String): StepResult {
        ...
        val cell: WebElement = table.findElement(By.xpath(".//tr[$row]/td[$column]"))
            ?: return StepResult.fail(
                "EXPECTED cell at Row $row Column $column of " +
    ...
    private fun extractInputData(context: ExecutionContext, row: Map<String, String>, dataOption: InputOptions) =
        if (row["tag"] == "select") {
            ...
            when (row["type"]) {
                "checkbox" -> if (StringUtils.equals(
                        Objects.toString(row["checked"]),
                        "true",
                    )
                ) {
                    "checked"
    ...
    private fun resolveDisplayText(cell: WebElement, isHeader: Boolean): String {
        ...
        val dataOption = InputOptions.valueOf(
            if (isHeader) {
                context.getStringData(HEADER_INPUT, getDefault(HEADER_INPUT))
            } else {
                context.getStringData(DATA_INPUT, getDefault(DATA_INPUT))
            },
        )
        ...
        if (selects.isNotEmpty()) {
            ...
            return if (firstElement.isDisplayed) {
                if (dataOption == InputOptions.state || dataOption == InputOptions.value) {
                    Select(firstElement).allSelectedOptions.joinToString(
                        separator = context.textDelim,
                        transform = { it.text },
                    )
                } else {
                    StringUtils.defaultString(firstElement.getAttribute(dataOption.toString()))
                }
            } else {
                ""
            }
            ...
            return if (firstElement.isDisplayed) {
                if (dataOption == InputOptions.state) {
                    when (firstElement.getAttribute("type")) {
                        "checkbox" -> if (StringUtils.equals(
                                firstElement.getAttribute("checked"),
                                "true",
                            )
                        ) {
                            "checked"
        ...
        if (images.isNotEmpty()) {
            val imageOption = ImageOptions.valueOf(
                if (isHeader) {
                    context.getStringData(HEADER_IMAGE, getDefault(HEADER_IMAGE))
                } else {
                    context.getStringData(DATA_IMAGE, getDefault(DATA_IMAGE))
                },
            )
