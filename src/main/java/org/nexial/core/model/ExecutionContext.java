...
/**
 * represent the state of a test execution. Differ from {@link ExecutionDefinition}, it contains the
 * derived test script and test data details, along with the thread-bound test context whereby a
 * test run is being maintained.
 */
public class ExecutionContext {
  // data variables that are READ-ONLY and cannot be removed/altered
  private static final List<Class> SIMPLE_VALUES =
      Arrays.asList(
          Boolean.class,
          Byte.class,
          Short.class,
  ...
  private static final String NON_DELIM_CHARS =
  ...
  private static final String NAME_CURRENT_COMMAND_PROFILES =
      "nexialInternal.currentCommandProfiles";
  ...
  class Function {
    ...
    void parse(String token) {
      ...
      // get the rest of token after the first pipe.  this should represent the entirety of all
      // params
      ...
      for (int i = 0; i < paramList.size(); i++) {
        ...
        // return the magic
        param =
            StringUtils.replace(param, TOKEN_TEMP_DELIM, escapedDelim ? TOKEN_PARAM_SEP : delim);
  ...
  public ExecutionContext(ExecutionDefinition execDef) {
    this(execDef, null);
  }
  ...
  public ExecutionContext(ExecutionDefinition execDef, Map<String, Object> intraExecutionData) {
    ...
    if (MapUtils.isNotEmpty(intraExecutionData)) {
      ...
      springContext =
          (ClassPathXmlApplicationContext) intraExecutionData.remove(NAME_SPRING_CONTEXT);
      initSpringBeans();

      plugins = (PluginManager) intraExecutionData.remove(NAME_PLUGIN_MANAGER);
      plugins.setContext(this);

      currentCommandProfiles =
          (Map<String, String>) intraExecutionData.remove(NAME_CURRENT_COMMAND_PROFILES);
    ...
    if (!ExecUtils.isRunningInZeroTouchEnv()
        && getBooleanData(OPT_ODI_ENABLED, getDefaultBool(OPT_ODI_ENABLED))) {
  ...
  public void logCurrentStep(String message) {
    ...
    if (isVerbose()) {
      currentTestStep.addNestedMessage(message);
    }
  }
  ...
  public ExecutionDefinition getExecDef() {
    return execDef;
  }

  public String getRunId() {
    return execDef.getRunId();
  }
  ...
  public String getId() {
    return "["
        + getRunId()
        + "]["
        + (testScript == null ? "UNKNOWN SCRIPT" : testScript.getFile().getName())
        + "]";
  }
  ...
  public Map<BrowserType, String> getWebdriverHelperConfig() {
    return webdriverHelperConfig;
  }

  public Map<String, String> getDbdriverHelperConfig() {
    return dbdriverHelperConfig;
  }

  public NexialS3Helper getOtc() throws IOException {
    // check that the required properties are set
    if (otc == null || !otc.isReadyForUse()) {
      throw new IOException(RB.Tools.text("otc.not.ready"));
    }
    return otc;
  }
  ...
  public NexialMailer getNexialMailer() {
    return nexialMailer;
  }
  ...
  public boolean isPluginLoaded(String target) {
    return plugins.isPluginLoaded(target);
  }
  ...
  public Browser getBrowser() {
    return plugins.initBrowser(currentCommandProfiles.get("web"));
  }

  public void closeBrowser(String profile) {
    if (StringUtils.isBlank(profile)) {
      profile = CMD_PROFILE_DEFAULT;
    }

    // clear references to browser for this profile, so that next use will (re)initialize browser
    // reference
  ...
  public MobileService getMobileService(String profile) {
    ...
    if (mobileService == null) {
      ...
      if (isVerbose()) {
        getLogger().log(getCurrentTestStep(), "" + manifest);
      }
  ...
  public Map<String, MobileService> getMobileServices() {
    return mobileServices;
  }

  public Excel getTestScript() {
    return testScript;
  }
  ...
  public ExecutionEventListener getExecutionEventListener() {
  ...
  public void registerScreenshotAgent(CanTakeScreenshot agent) {
    screenshotAgent = agent;
  }

  public void clearScreenshotAgent() {
    screenshotAgent = null;
  }

  public CanTakeScreenshot findCurrentScreenshotAgent() {
  ...
  /**
   * "fail-fast" is stifled for Nexial Interactive so that we can uncover more issues during
   * interactive mode.
   */
  public boolean isFailFast() {
    return getBooleanData(FAIL_FAST, getDefaultBool(FAIL_FAST)) && !isInteractiveMode();
  }

  public boolean isResolveTextAsURL() {
    return hasData(RESOLVE_TEXT_AS_URL)
        ? getBooleanData(RESOLVE_TEXT_AS_URL)
        : hasData(Expression.EXPRESSION_RESOLVE_URL)
            ? getBooleanData(Expression.EXPRESSION_RESOLVE_URL)
            : getDefaultBool(RESOLVE_TEXT_AS_URL);
  }

  public boolean isResolveTextAsIs() {
    return hasData(RESOLVE_TEXT_AS_IS)
        ? getBooleanData(RESOLVE_TEXT_AS_IS)
        : hasData(Expression.EXPRESSION_OPEN_FILE_AS_IS)
            ? getBooleanData(Expression.EXPRESSION_OPEN_FILE_AS_IS)
            : getDefaultBool(RESOLVE_TEXT_AS_IS);
  }

  /**
   * increment current failure count and evaluate if execution failure should be declared since
   * we've passed the failAfter threshold
   */
  public void incrementAndEvaluateFail(StepResult result) {
    ...
    // in effect, the `onError()` event works the same way as `pauseOnError`. Nexial supports both
    // stylistic variety
    executionEventListener.onError();

    if (isPauseOnError()) {
      ConsoleUtils.doPause(
          this,
          "[ERROR #"
              + execFailCount
              + "]: Error found "
  ...
  public int getFailAfter() {
    return getIntData(FAIL_AFTER, getDefaultInt(FAIL_AFTER));
  }

  public boolean isStepByStep() {
    return getBooleanData(OPT_STEP_BY_STEP);
  }

  public boolean isVerbose() {
    return getBooleanData(VERBOSE);
  }
  ...
  public boolean isTextMatchCaseInsensitive() {
    return getBooleanData(
        OPT_TEXT_MATCH_CASE_INSENSITIVE, getDefaultBool(OPT_TEXT_MATCH_CASE_INSENSITIVE));
  }

  // public boolean isProxyRequired() { return getBooleanData(OPT_PROXY_REQUIRED, false); }

  public boolean isOutputToCloud() {
    return getBooleanData(OUTPUT_TO_CLOUD, getDefaultBool(OUTPUT_TO_CLOUD));
  }
  ...
  @NotNull
  public String getTextDelim() {
    return getRawStringData(TEXT_DELIM, ",");
  }

  @NotNull
  public String getNullValueToken() {
    return getRawStringData(NULL_VALUE, NULL);
  }

  public long getPollWaitMs() {
    return getIntConfig("web", currentCommandProfiles.get("web"), POLL_WAIT_MS);
  }

  public long getSLAElapsedTimeMs() {
    return getIntData(OPT_ELAPSED_TIME_SLA);
  }
  ...
  public boolean useExplicitWait(String profile) {
    ...
    if (hasConfig("web", profile, WEB_ALWAYS_WAIT)) {
      return getBooleanConfig("web", profile, WEB_ALWAYS_WAIT);
    }
    return getDefaultBool(WEB_EXPLICIT_WAIT);
  }

  public boolean hasData(String name) {
    return getObjectData(name) != null;
  }
  ...
  public String getStringData(String name) {
    ...
    if (StringUtils.startsWith(rawValue, CRYPT_IND)) {
      CellTextReader.registerCrypt(name, rawValue, resolved);
    }
    return resolved;
  }
  ...
  public int getIntData(String name) {
    ...
    if (StringUtils.isBlank(value)) {
      return UNDEFINED_INT_DATA;
    }
    try {
      return Integer.parseInt(value);
    } catch (NumberFormatException e) {
      throw new IllegalArgumentException(
          "Unable to parse data '" + name + "' as int (" + value + ")", e);
    }
  }

  public int getIntData(String name, int def) {
    return NumberUtils.toInt(getStringData(name), def);
  }
  ...
  public double getDoubleData(String name, double def) {
    return NumberUtils.toDouble(getStringData(name), def);
  }

  public boolean getBooleanData(String name) {
    return BooleanUtils.toBoolean(getStringData(name));
  }
  ...
  /**
   * return a map of name-value pair where the names start with the specified {@code prefix}. Note
   * that the returned names have the {@code prefix} removed.
   */
  @NotNull
      ...
      data.forEach(
          (key, value) -> {
            if (StringUtils.startsWith(key, prefix1)) {
              props.put(
                  StringUtils.substringAfter(key, prefix1), replaceTokens(Objects.toString(value)));
            }
          });
          ...
          .forEach(
              (key, value) -> {
                ...
                if (StringUtils.startsWith(sKey, prefix1)) {
                  props.put(
                      StringUtils.substringAfter(sKey, prefix1),
                      replaceTokens(Objects.toString(value)));
                }
              });
    }
      ...
      data.forEach(
          (key, value) -> {
            if (StringUtils.startsWith(key, prefix)) {
              props.put(
                  StringUtils.substringAfter(key, prefix), replaceTokens(Objects.toString(value)));
            }
          });
          ...
          .forEach(
              (key, value) -> {
                ...
                if (StringUtils.startsWith(sKey, prefix)) {
                  props.put(
                      StringUtils.substringAfter(sKey, prefix),
                      replaceTokens(Objects.toString(value)));
                }
              });
    }
   ...
   *
   * <p>Note that the keys of the returning map are _WITHOUT_ {@literal prefix}.
   */
  @NotNull
  ...
  public void setData(String name, String value) {
    // some reference data are considered "special" and should be elevated to "execution" level so
    // that they can be
  ...
  public void setData(String name, String value, boolean updateSysProps) {
    ...
    if (data.containsKey(name)) {
      CellTextReader.unsetValue(value);
    }
    if (isNullValue(value)) {
      ...
      // if (updateSysProps || referenceDataForExecution.contains(name)) { System.setProperty(name,
      // value); }
      if (updateSysProps) {
  ...
  public void setData(String name, List value) {
    setDataOrSysProp(name, value);
  }
  ...
  public boolean isNullValue(String value) {
    return value == null || StringUtils.equals(value, getNullValueToken());
  }
  ...
  public boolean isBlankValue(String value) {
    return StringUtils.isBlank(value) || StringUtils.equals(value, BLANK);
  }

  public boolean isNullOrEmptyValue(String value) {
    return isNullValue(value) || isEmptyValue(value);
  }
  ...
  public static boolean asNull(String value) {
    return value == null || StringUtils.equals(value, NULL);
  }
  ...
  public static boolean asNullOrEmpty(String value) {
    return asNull(value) || asEmpty(value);
  }
  ...
  public String replaceTokens(String text) {
    return replaceTokens(text, false);
  }
  ...
  public String replaceTokens(String text, boolean retainCrypt) {
    ...
    if (StringUtils.equals(text, getNullValueToken())) {
      return null;
    }
    ...
    List<String> ignoredVars =
        TextUtils.toList(getRawStringData(OPT_VAR_EXCLUDE_LIST), getTextDelim(), false);
    if (CollectionUtils.isNotEmpty(ignoredVars)) {
      ignoredVars.forEach(tokens::remove);
    }
    ...
    if (hasData(OPT_VAR_DEFAULT_AS_IS)) {
      ...
      if (config != null) {
        unresolvedAsIs = BooleanUtils.toBoolean(config.toString());
      }
    }
    ...
    for (String token : tokens) {
      ...
      if (value == null || value.equals(NULL)) {
        ...
        if (data.containsKey(token)) {
          ...
          // if not, replace token with "" and continue.  Doesn't make sense to replace token with
          // `null`
        ...
        // otherwise, we skip this token (meaning no value was assigned to this token).  Most likely
        // this
      ...
      if (NON_PRINTABLE_REPLACEMENTS.containsKey(value.toString())) {
        text =
            StringUtils.replace(text, tokenized, NON_PRINTABLE_REPLACEMENTS.get(value.toString()));
        continue;
      }
      ...
      if (valueType.isPrimitive() || SIMPLE_VALUES.contains(valueType)) {
        ...
        if (retainCrypt && CellTextReader.isCrypt(stringValue)) {
          stringValue = tokenized;
        }

        // it's possible that we might get a reference to an item of a string-based array (like
        // "a,b,c,d")
        ...
        if (isListCompatible(stringValue)) {
          ...
          if (hasIndexRef) {
            ...
            List<String> indexRefs =
                RegexUtils.eagerCollectGroups(text, regexIndexRef, false, true);
            for (String indexRef : indexRefs) {
              ...
              if (!NumberUtils.isDigits(indexStr2)) {
                continue;
              }
    ...
    if (CollectionUtils.isNotEmpty(tokens) && allTokenResolvedToNull) {
      ...
      String tmp = text;
      for (String token : tokens) {
        tmp = StringUtils.remove(tmp, TOKEN_START + token + TOKEN_END);
      }
    ...
    if (MapUtils.isNotEmpty(complexValues)) {
      text = replaceComplexTokens(text, complexValues);
    }
    ...
    // sixth pass: crypt
    if (StringUtils.startsWith(text, CRYPT_IND) && !retainCrypt) {
      text = CellTextReader.getText(text);
    }
  ...
  public String handleExpression(String text) {
    ...
    try {
      ...
      String errorMessage =
          "Unable to process expression due to "
      ...
      if (cause instanceof IOException) {
        throw new IllegalArgumentException(
            "Unable to process expression due to " + cause.getMessage());
      }
  ...
  public String handleFunction(String text) {
    ...
    while (StringUtils.isNotBlank(functionToken)) {
      // put back the temporarily replace $(...), we need invokeFunction() to work on the original
      // data
      ...
      text =
          StringUtils.replaceOnce(
              text, TOKEN_FUNCTION_START + functionToken + TOKEN_FUNCTION_END, value);
  ...
  public String getCurrentScenario() {
    ...
    try {
      return currentTestStep != null
          ? currentTestStep.getTestCase().getTestScenario().getName()
          : null;
  ...
  protected void setCurrentTestStep(TestStep testStep) {
  ...
  public String getCurrentActivity() {
    return getStringData(OPT_CURRENT_ACTIVITY);
  }
  ...
  public Map<String, String> gatherScenarioReferenceData() {
    return gatherReferenceData(SCENARIO_REF_PREFIX);
  }

  public Map<String, String> gatherScriptReferenceData() {
    return gatherReferenceData(SCRIPT_REF_PREFIX);
  }
  ...
  public void clearScenarioRefData() {
    clearReferenceData(SCENARIO_REF_PREFIX);
  }

  public void addScriptReferenceData(String name, String value) {
    addReferenceData(SCRIPT_REF_PREFIX, name, value);
  }

  public void clearScriptRefData() {
    clearReferenceData(SCRIPT_REF_PREFIX);
  }

  public void startIteration(
      int iterationIndex, int iterationRef, int totalIterationCount, boolean firstUse) {
    getTrackTimeLogs();

    // remember whether we want to track execution completion as a time-track event or not
    System.setProperty(
        TRACK_EXECUTION, getStringData(TRACK_EXECUTION, getDefault(TRACK_EXECUTION)));
    ...
    if (firstUse) {
      eventListener.onExecutionStart();
    }
    if (iterationIndex == 1) {
      eventListener.onScriptStart();
    }
    eventListener.onIterationStart();
  }

  public void endIteration() {
    if (getBooleanData(OPT_INTERACTIVE, false)) {
      return;
    }
  ...
  public void endScript() {
    ...
    if (testScript != null) {
      try {
        ...
        ConsoleUtils.error(
            "Unable to close script (" + testScript.getOriginalFile() + "): " + e.getMessage());
      }
  ...
  public int getScriptStepCount() {
    return scriptStepCount;
  }

  public int getScriptPassCount() {
    return scriptPassCount;
  }

  public int getScriptFailCount() {
    return scriptFailCount;
  }

  public int getScriptWarnCount() {
    return scriptWarnCount;
  }

  // support flow controls - FailIf()
  public boolean isFailImmediate() {
    return getBooleanData(FAIL_IMMEDIATE, false);
  }

  public void setFailImmediate(boolean failImmediate) {
    data.put(FAIL_IMMEDIATE, failImmediate);
  }

  // support flow controls - EndIf()
  public boolean isEndImmediate() {
    return getBooleanData(END_IMMEDIATE, false);
  }

  public void setEndImmediate(boolean endImmediate) {
    data.put(END_IMMEDIATE, endImmediate);
    if (!ExecUtils.IS_RUNNING_IN_JUNIT) {
      System.setProperty(END_SCRIPT_IMMEDIATE, endImmediate + "");
    }
  }

  // support flow controls - EndIf()
  public boolean isMacroStepFailed() {
    return getBooleanData(MACRO_STEP_FAILED, false);
  }

  public void setMacroStepFailed(boolean stepFailed) {
    data.put(MACRO_STEP_FAILED, stepFailed);
  }

  public void setInMacroFlex(boolean isInMacro) {
    this.isInMacro = isInMacro;
  }

  // support flow controls - EndIf()
  public boolean isMacroBreakIteration() {
    return getBooleanData(MACRO_BREAK_CURRENT_ITERATION, false);
  }

  public void setMacroBreakIteration(boolean isMacroBreak) {
    data.put(MACRO_BREAK_CURRENT_ITERATION, isMacroBreak);
  }
  ...
  public void setBreakCurrentIteration(boolean breakLoop) {
    setData(BREAK_CURRENT_ITERATION, breakLoop);
  }
  ...
  public TrackTimeLogs getTrackTimeLogs() {
    ...
    if (obj instanceof TrackTimeLogs) {
      trackTimeLogs = (TrackTimeLogs) obj;
    }
  ...
  public void removeTrackTimeLogs() {
    removeData(NAME_TRACK_TIME_LOGS);
  }
  ...
  public boolean execute() throws IOException {
    ...
    // gather pre-execution reference data here, so that after the execution we can reset the
    // reference data
  ...
  public boolean containsCrypt(String data) {
    try {
      if (StringUtils.contains(data, CRYPT_IND)) {
        return true;
      }
      ...
      if (CellTextReader.isCrypt(data)) {
        return true;
      }
      ...
      return CollectionUtils.isNotEmpty(variables)
          && variables.stream()
              .anyMatch(
                  var -> CellTextReader.isCrypt(replaceTokens(TOKEN_START + var + TOKEN_END)));
    } catch (RuntimeException e) {
      // don't worry it now. Nexial will eventually barf at such exception later on (and probably at
      // a better spot).
      return false;
    }
  }

  public static boolean getSystemThenContextBooleanData(
      String name, ExecutionContext context, boolean def) {
  ...
  public static String getSystemThenContextStringData(
      String name, ExecutionContext context, String def) {
  ...
  public static String unescapeToken(String text) {
    return StringUtils.replace(
        StringUtils.replace(text, ESCAPED_TOKEN_START, TOKEN_START), ESCAPED_TOKEN_END, TOKEN_END);
  }

  public static String escapeToken(String text) {
    return StringUtils.replace(
        StringUtils.replace(text, TOKEN_START, ESCAPED_TOKEN_START), TOKEN_END, ESCAPED_TOKEN_END);
  }

  /**
   * resolve a set of name-value configuration, based on a "profile", and store it in the current
   * {@link ExecutionContext context}. This is useful for profile-aware commands where multiple
   * instances of the same command co-exist and distinguish by the associated "profile".
   */
   ...
   * instances of the same command co-exist and distinguish by the associated "profile".
   */
  public Map<String, String> updateProfileConfig(
      String command, String profile, Map<String, String> config) {
  ...
  public Map<String, String> getProfileConfig(String command, String profile) {
    ...
    if (!hasData(configMapKey)) {
      return config;
    }
    if (!(getObjectData(configMapKey) instanceof Map)) {
      return config;
    }

    Map mapData = getMapData(configMapKey);
    TypeVariable<? extends Class<? extends Map>>[] mapTypes =
        mapData.getClass().getTypeParameters();
  ...
  public String truncateForDisplay(String text) {
    return truncateForDisplay(
        text, getIntData(MAX_CONSOLE_DISPLAY, getDefaultInt(MAX_CONSOLE_DISPLAY)));
  }
  ...
  protected String handleCryptValue(String text) {
    ...
    while (StringUtils.isNotBlank(crypt)) {
      ...
      // decryption failed?
      if (StringUtils.equals(crypt, decrypt)) {
        return text;
      }
  ...
  /**
   * updating corresponding System property when a data variable of the same name is updated. Note
   * that the purpose of this method is to keep in sync the data variable that might have been set
   * as System variable with those defined in data sheets. Data variables defined via command line (
   * <code>-Dname=value</code>, for example) and <code>project.properties</code> are kept as System
   * properties. Suppose the same data variable is updated with new value during execution, then we
   * need to update both the data variable repo ({@link #data}) and System properties to keep the
   * same data variable in sync.
   *
   * <p>Note that System properties will be updating <b>ONLY</b> IF:
   *
   * <ol>
   *   <li>{@code name} is valid (not null)
   *   <li>the same data variable is already defined as System properties (hence needing to
   *       overwrite)
   *   <li>execution is currently in progress
   * </ol>
   */
  protected void setDataOrSysProp(String name, Object value) {
    ...
    if (!isNullValue) {
      if (data.containsKey(name)) {
        CellTextReader.unsetValue(Objects.toString(value));
      }
      ...
      if (value.getClass().isArray() && ArrayUtils.getLength(value) < 1) {
        remove = true;
      }
    } else {
      remove = true;
    ...
    if (StringUtils.isEmpty(System.getProperty(name))) {
      ...
      if (referenceDataForExecution.contains(name)) {
  ...
  protected String getRawStringData(String name) {
    return System.getProperty(name, flattenAsString(data.get(name)));
  }

  protected String getRawStringData(String name, String def) {
    return StringUtils.defaultString(
        System.getProperty(name), data.containsKey(name) ? flattenAsString(data.get(name)) : def);
  }

  protected boolean isListCompatible(String value) {
    String delim = getTextDelim();
    return StringUtils.isNotBlank(value)
        && !StringUtils.equals(value, delim)
        && StringUtils.contains(value, delim);
  }

  protected boolean containListAccess(String text, String tokenized) {
    if (StringUtils.isEmpty(text) || StringUtils.isEmpty(tokenized)) {
      return false;
    }
  ...
  protected void clearReferenceData(String prefix) {
    Object[] deleteCandidates =
        data.keySet().stream().filter(key -> StringUtils.startsWith(key, prefix)).toArray();
    for (Object deleteCandidate : deleteCandidates) {
      data.remove(Objects.toString(deleteCandidate));
    }
  ...
  protected Map<String, String> gatherReferenceData(String prefix) {
    return getDataByPrefix(prefix);
  }
  ...
  protected String replaceCollectionTokens(
      String text, Map<String, Object> collectionValues, Map<String, Object> complexValues) {
      ...
      while (StringUtils.contains(text, token)) {
        ...
        if (!StringUtils.startsWith(postToken, TOKEN_ARRAY_START)) {
          ...
          if (StringUtils.startsWith(postToken, ".")) {
            ...
            String propName =
                StringUtils.replaceChars(postToken.substring(1), ".;:|,\t\r\n", "        ");
            ...
            if (replaceBy == null) {
              text =
                  StringUtils.equals(text, searchBy)
                      ? null
                      : StringUtils.replace(text, searchBy, getNullValueToken());
            } else {
   ...
   *
   * <p>we want to implement a dot-style object reference so that we can interrogate nested object
   * property of an object. For example, a.b.c.d would be something like a.getB().getC().getD().
   * However, the dot character might be used as a part of the "text" parameter as a legitimate
   * character. We would use escape-dot ( \. ) to represent a dot literal.
   *
   * <p>Strategy: 1) replace dot-literal with another character sequence (DOT_LITERAL_REPLACER) 2)
   * find next token prior to dot 3) replace token with value 4) recurse next token set with the
   * derived object 5) at the last recursion, replace DOT_LITERAL_REPLACER back to dot-literal
   */
  protected String replaceComplexTokens(String text, Map<String, Object> complexValues) {
    ...
    // todo: we need to consider storing resulting object as complex type, while still returning
    // string
  ...
  protected String replaceNestedTokens(String text, Map<String, String> data, Object value) {
    ...
    if (value == null) {
      return StringUtils.replace(text, token, "");
    }

    String postToken = data.get(NAME_POST_TOKEN);

    // if value is simple type, then no more introspection can be done.
    // can't find the ${data}.xyz or ${data}.[xyz] pattern.  just replace ${data} with value
    // stringified
    ...
    if ((value.getClass().isArray() || Collection.class.isAssignableFrom(value.getClass()))
        && StringUtils.startsWith(postToken, TOKEN_ARRAY_START)
        && StringUtils.contains(postToken, TOKEN_ARRAY_END)) {

      String propName =
          StringUtils.substring(postToken, 1, StringUtils.indexOf(postToken, TOKEN_ARRAY_END, 1));
      if (!NumberUtils.isDigits(propName)) {
      ...
      if (newValue == null) {
        return flattenAsString(text, data.get(NAME_TOKEN), value);
      }
    ...
    if (StringUtils.startsWith(postToken, "." + TOKEN_ARRAY_START)) {
      // find next .[prop]
      propName =
          StringUtils.substring(postToken, 2, StringUtils.indexOf(postToken, TOKEN_ARRAY_END, 2));
      token += "." + TOKEN_ARRAY_START + propName + TOKEN_ARRAY_END;
      postToken = StringUtils.substringAfter(postToken, propName + TOKEN_ARRAY_END);
    } else {
      // find next .prop, which should ends before next dot or whitespace, or EOL.
      int propDelimIndex =
          StringUtils.indexOfAnyBut(StringUtils.substring(postToken, 1), NON_DELIM_CHARS);
      if (propDelimIndex == 0) {
        return flattenAsString(text, token, value);
      }
      if (propDelimIndex < 1) {
      ...
      if (Map.class.isAssignableFrom(value.getClass())) {
        ...
        if (Collection.class.isAssignableFrom(value.getClass())) {
          ...
          if (PropertyUtils.isReadable(value, propName)) {
            ...
            String args =
                StringUtils.substringBetween(postToken, TOKEN_ARRAY_START, TOKEN_ARRAY_END);
            newValue =
                ObjectUtils.defaultIfNull(
                    MethodUtils.invokeMethod(
                        value, propName, StringUtils.split(args, getDefault(TEXT_DELIM))),
                    "");
  ...
  protected String invokeFunction(String token) {
    if (StringUtils.countMatches(token, TOKEN_PARAM_SEP) < 1) {
      throw new IllegalArgumentException(
          "reference to a built-in function NOT shown via the $(...|...) format");
    }

    String errorPrefix =
        "Invalid built-in function " + TOKEN_FUNCTION_START + token + TOKEN_FUNCTION_END + " ";
    ...
    try {
      ...
      return Objects.toString(
          MethodUtils.invokeExactMethod(f.function, f.operation, f.parameters), "");
      ...
      if (cause != null) {
        throw new RuntimeException(
            errorPrefix + "error on '" + f.operation + "': " + cause.getMessage(), e);
      } else {
        throw new RuntimeException(
            errorPrefix + "error on '" + f.operation + "': " + e.getMessage(), e);
      }
    }
  }
  ...
  protected String nextFunctionToken(String text) {
    ...
    if (!StringUtils.contains(text, TOKEN_FUNCTION_START)) {
      return null;
    }

    String tokenStart = StringUtils.substringAfterLast(text, TOKEN_FUNCTION_START);
    if (!StringUtils.contains(tokenStart, TOKEN_FUNCTION_END)) {
      return null;
    }
    ...
    // in order to prevent re-surfacing the bad $(...) pattern, we'll replace it with something else
    // and switch
    // to its initial form during invokeFunction()

    // keep search for $(...) pattern
    return nextFunctionToken(
        text.replace(
  ...
  protected String resolveDeferredTokens(String text) {
    ...
    for (String token : tokens) {
      ...
      String replace =
          StringUtils.equals(searchToken, nullValue) ? null : replaceTokens(replaceToken);
      if (replace == null) {
  ...
  protected static String getIndexedRef(String value, int expectedArrayStartIdx) {
    ...
    if (startArrayIdx != -1 && endArrayIdx != -1 && expectedArrayStartIdx == startArrayIdx) {
      String listIndex =
          StringUtils.substring(value, startArrayIdx + TOKEN_ARRAY_START.length(), endArrayIdx);
  ...
  protected void clearCurrentTestStep() {
  ...
  protected TestProject adjustPath(ExecutionDefinition execDef) {
    ...
    project.setOutPath(
        StringUtils.appendIfMissing(project.getOutPath(), separator) + execDef.getRunId());
  ...
  protected void parse() throws IOException {
    // parse and collect all relevant test data so we can merge then into iteration-bound test
    // script
    ...
    for (int i = 1; i < endRowIndex; i++) {
      ...
      // no longer checks for blank value, we'll accept data value as is since it's been dealt with
      // in
    ...
    if (StringUtils.isBlank(System.getProperty(OPT_OPEN_RESULT))) {
      boolean openResult =
          MapUtils.getBoolean(data, OPT_OPEN_RESULT, getDefaultBool(OPT_OPEN_RESULT));
    ...
    System.setProperty(
        TARGET_DISPLAY, MapUtils.getString(data, TARGET_DISPLAY, getDefault(TARGET_DISPLAY)));
  ...
  private void initSpringBeans() {
    failfastCommands =
        springContext.getBean("failfastCommands", new ArrayList<String>().getClass());

    // init built-in variables
    builtinFunctions =
        springContext.getBean("builtinFunctions", new HashMap<String, Object>().getClass());
    ...
    executionEventListener =
        springContext.getBean("executionEventListener", ExecutionEventListener.class);
    ...
    referenceDataForExecution =
        springContext.getBean("nexial.referenceDataForExecution", List.class);
    readOnlyVars = springContext.getBean("readOnlyVars", new ArrayList<String>().getClass());

    // web driver
    webdriverHelperConfig =
        springContext.getBean(
            "webdriverHelperConfig", new HashMap<BrowserType, String>().getClass());
  ...
  private void overrideIfSysPropFound(String propName) {
    if (StringUtils.isNotBlank(System.getProperty(propName))) {
      setData(propName, System.getProperty(propName));
    }
  }
    ...
    if (value instanceof Map) {
      ...
      if (MapUtils.isNotEmpty(map)) {
        for (Object key : map.keySet()) {
          buffer
              .append(flattenAsString(key))
              .append(mapSep)
              .append(flattenAsString(map.get(key)))
              .append(groupSep);
        }
      }
  ...
  private String flattenArrayOfObject(Object value, String property) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      try {
        ...
        // if all value instances of the same prop are also null, then we will return just null as
        // well
    ...
    if (StringUtils.equals(valueItem, nullValue)) {
      return null;
    }
  ...
  private String flattenCollection(Collection value, String name) {
    ...
    // it's possible that we are looking for an "object.prop" replacement. We should check if
    // "value" is a
    ...
    if (isMapType) {
      ...
      for (Object item : value) {
        ...
        // special treatment for null value(s)
        // we will return null value as is (not as string 'null');
        // if all value instances of the same key/name are also null, then we will return just null
        // as well
      ...
      // if there's no such value (based on key/name) or all of them are null, then we'll just
      // return null
      if (buffer.length() == 0 || onlyNull) {
        return null;
      }
    ...
    try {
      ...
      try {
        ...
        ConsoleUtils.error(
            "Unable to derive '" + name + "' from value " + value + ": " + e.getMessage());
        return null;
      }
    }
  }
  ...
  private String adjustForMacroFlex(String name) {
    ...
    if (System.getProperties().containsKey(MACRO_FLEX_PREFIX + name)) {
      return MACRO_FLEX_PREFIX + name;
    }
    if (data.containsKey(MACRO_FLEX_PREFIX + name)) {
      return MACRO_FLEX_PREFIX + name;
    }
    return name;
  }
  ...
  private boolean prefixedForMacroFlex(String name) {
    return !StringUtils.startsWith(name, NAMESPACE) && isInMacro;
  }
}
