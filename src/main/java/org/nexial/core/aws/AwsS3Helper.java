...
public class AwsS3Helper {
  ...
  public void setAssumeRoleSession(String assumeRoleSession) {
    this.assumeRoleSession = assumeRoleSession;
  }

  public void setAssumeRoleDuration(int assumeRoleDuration) {
  ...
  public PutObjectResult copyToS3(File file, PutOption options) {
    ...
    String objectPath =
        (subDir != null ? StringUtils.appendIfMissing(subDir, "/") : "") + file.getName();
    PutObjectRequest request = new PutObjectRequest(bucketName, objectPath, file);
    if (options != null) {
      if (options.isPubliclyReadable()) {
        request = request.withCannedAcl(PublicRead);
      }
      if (options.isReducedRedundancy()) {
  ...
  public String importToS3(File source, String targetPath, boolean removeLocal) throws IOException {
    ...
    if (result == null) {
      throw new IOException("Unable to import to S3 successfully");
    }
   ...
   * @param removeFromBucket flag to check whether to delete the file after sending the contents or
   *     not.
   ...
   */
  public byte[] copyFromS3(
      @NotNull final String bucket, @NotNull final String key, final boolean removeFromBucket)
      throws IOException {
    assert StringUtils.isNotBlank(accessKey);
    assert StringUtils.isNotBlank(secretKey);
  ...
  public static String toPattern(String path) {
    ...
    String delim =
        (StringUtils.startsWith(path, REGEX_PREFIX)) ? REGEX_PREFIX : (S3_PATH_SEP + REGEX_PREFIX);
    int delimLength = delim.length();

    int regexStartPos = StringUtils.indexOf(path, delim);
    if (regexStartPos != -1) {
      return (regexStartPos > 0
              ? toSimplePattern(StringUtils.substring(path, 0, regexStartPos + 1))
              : "")
  ...
  public List<String> listFiles(@NotNull final String s3Path) {
    ...
    if (!StringUtils.contains(path, REGEX_PREFIX)) {
      ...
      if (!StringUtils.contains(path, "*")) {
        return getFileKeys(REGEX_FOR_ANY);
      }
  ...
  /**
   * added to circumvent MITM cert injection (possibly instrumented by corporate infosec/firewall
   * team)
   */
  static void beforeS3Connection() {
    System.setProperty(
        "BEFORE::" + DISABLE_CERT_CHECKING_SYSTEM_PROPERTY,
  ...
  protected List<String> getFileKeys(@NotNull final String pattern) {
    ...
    // if subDir is empty
    Predicate<S3ObjectSummary> keyFilter =
        summary -> {
          ...
          //        StringUtils.startsWith(key, subDir) : StringUtils.endsWith(key,
          // S3_PATH_SEPARATOR);
        };
    ...
    while (true) {
      ...
      if (StringUtils.isNotEmpty(subDir)) {
        request = request.withPrefix(subDir);
      }
  ...
  protected static String toSimplePattern(String path) {
    String regex = path;
    for (char ch : REGEX_ESCAPE_CHARS) {
      regex = StringUtils.replace(regex, ch + "", "\\" + ch);
    }
  ...
  private AmazonS3 newS3Client() {
    return newS3Client(region == null ? DEFAULT_REGION : region);
  }
  ...
  private AmazonS3 newS3Client(@NotNull final Regions region) {
    ...
    if (StringUtils.isNotEmpty(this.url)) {
      s3ClientBuilder.withEndpointConfiguration(getEndpointConfig());
    } else {
  ...
  private AWSCredentialsProvider resolveCredentials(Regions region) {
    ...
    return AwsSupport.resolveAssumeRoleCredentials(
        credProvider, region, assumeRoleArn, assumeRoleSession, assumeRoleDuration);
  }
  ...
  private byte[] getS3ObjectContent(@NotNull final S3Object s3Object) throws IOException {
    ...
    try (BufferedInputStream bis = new BufferedInputStream(s3Object.getObjectContent());
